# Task 2 Attempt 9: T5-base + type-conditioned input.
# Same proven config as Attempt 3 (512 length, batch4/accum4, lr1e-4, 10 epochs) —
# the ONLY change is adding "type: {phrase/passage/multi}" to the input prompt,
# so the model gets an explicit signal about what kind of answer to produce.
# Uses the TRUE tags field (train/val both have it) for this experiment.
...
DRIVE_DIR = '/content/drive/MyDrive/MSE641_Task2_TypeCond'
import os
...
def build_input(row):
    spoiler_type = row['tags'][0]
    post = ' '.join(row['postText'])
    title = row['targetTitle']
    paragraphs = ' '.join(row['targetParagraphs'])
    return 'type: ' + spoiler_type + ' question: ' + post + ' context: ' + title + ' ' + paragraphs
...
args = Seq2SeqTrainingArguments(
    output_dir='/content/task2-t5-typecond-output',
    num_train_epochs=10,
...
with open(f'{DRIVE_DIR}/task2-t5-typecond-val-preds.jsonl', 'w') as f:
    for item, pred in zip(val_data, decoded):
        f.write(json.dumps({'uuid': item['id'], 'spoiler': pred}) + '\n')
print(f'Val predictions saved to {DRIVE_DIR}/task2-t5-typecond-val-preds.jsonl')
